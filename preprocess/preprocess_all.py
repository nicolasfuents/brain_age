...
# Paralelismo dinámico adaptado a la fracción atómica de TRILLIUM
N_JOBS = 24
N_GPUS = 1
...
def robust_normalize_p1_p99(data, mask):
    """
    Armonización de contraste: Clipping P1-P99 y reescalado Min-Max a [0, 1].
    """
    ...
    clipped_vox = np.clip(vox, p01, p99)
    scaled_vox = (clipped_vox - p01) / (p99 - p01)

    out[valid] = scaled_vox
    out[~mask] = 0.0
...
# ==============================================================================
def process_subject(row, idx, in_dir, output_root, args, solid_mask, file_map=None):
    gpu_id = idx % N_GPUS

    # Resolución dinámica exhaustiva de la clave del sujeto
    if "Scan_ID" in row and pd.notna(row["Scan_ID"]):
        subject_id = str(row["Scan_ID"]).strip()
    elif "Subject_ID" in row and pd.notna(row["Subject_ID"]):
        subject_id = str(row["Subject_ID"]).strip()
    elif "ID" in row and pd.notna(row["ID"]):
        subject_id = str(row["ID"]).strip()
    elif "participant_id" in row and pd.notna(row["participant_id"]):
        subject_id = str(row["participant_id"]).strip()
    else:
        return f"[ERROR] Fila sin identificador válido (columnas detectadas: {list(row.keys())})"
    ...
    # Bypass de eficiencia: si los 3 tensores ya existen y están serializados, saltamos el cómputo
    if (t1_tensor_dir / "axial" / f"{subject_id}.pt").exists() and \
       (t1_tensor_dir / "coronal" / f"{subject_id}.pt").exists() and \
       (t1_tensor_dir / "sagittal" / f"{subject_id}.pt").exists():
        return f"[SKIP] {subject_id} - Tensores consolidados previamente"
    ...
    # BÚSQUEDA DEL ARCHIVO CON MAPA TOPOLÓGICO SEGURO
    warning_msg = ""
    if args.flat_input and file_map is not None:
        possible_keys = [subject_id, f"sub-{subject_id}", subject_id.replace("sub-", "")]
        raw_path = next((file_map[k] for k in possible_keys if k in file_map), None)
        if raw_path is None:
            return f"[ERROR] {subject_id}: No se encontró NIfTI mapeado"
    else:
        raw_path_dir = in_dir / subject_id
        if not raw_path_dir.exists():
            matches = [m for m in in_dir.glob(f"**/*{subject_id}*") if m.is_dir()]
            raw_path_dir = matches[0] if matches else None
            
        if raw_path_dir is None:
            return f"[ERROR] {subject_id}: No se encontró subcarpeta en {in_dir}"

        raw_path, warning_msg = select_t1_file(raw_path_dir, args.filter, args.wildcard)
    ...
    try:
        ...
        # 6. Normalización P1-P99 robusta dentro de la máscara
        data_norm, val_p1, val_p99 = robust_normalize_p1_p99(data_masked, solid_mask)
        ...
        meta = {
            ...
            "p99": float(val_p99),
            "fslcc": float(cc_val),
            "harmonization": "brainprep_MNI152_N4 + SOLID_v2_mask + P1_P99_clip + MinMax[0,1] + TopoStd",
        }
...
def main():
    ...
    parser.add_argument("--in_dir", type=str, required=True, help="Directorio raíz de entrada")
    parser.add_argument("--csv", type=str, required=True, help="Archivo CSV de metadatos")
    parser.add_argument("--skip_brainprep", action="store_true", help="Omitir reorientación y N4")
    parser.add_argument("--flat_input", action="store_true", help="NIfTIs sueltos en in_dir")
    ...
    # Soporte para Slurm Job Arrays
    parser.add_argument("--array_id", type=int, default=None)
    parser.add_argument("--array_tasks", type=int, default=None)
    args = parser.parse_args()

    in_dir = Path(args.in_dir)
    output_root = in_dir / "processed_final_SOLID_V2_P1_P99"
    output_root.mkdir(parents=True, exist_ok=True)

    print("--- PROCESAMIENTO END-TO-END UNIFICADO (P1-P99) ---")

    df = pd.read_csv(args.csv)

    if args.array_id is not None and args.array_tasks is not None:
        chunks = np.array_split(df, args.array_tasks)
        df = chunks[args.array_id]
        print(f"[*] Slurm Array detectado: Procesando fragmento {args.array_id + 1} de {args.array_tasks}")

    # PRE-CÁLCULO DEL MAPA DE ARCHIVOS PARA DIRECTORIOS PLANOS
    file_map = None
    if args.flat_input:
        print("[*] Indexando estructura de directorio plano con limpieza de sufijos...")
        files = [f for f in os.listdir(in_dir) if f.endswith(".nii.gz") or f.endswith(".nii")]
        file_map = {}
        for fname in files:
            name_clean = fname.replace(".nii.gz", "").replace(".nii", "")
            # 1. Cortar antes de la etiqueta de procesamiento de brainprep
            extracted_id = name_clean.split("_desc-")[0] if "_desc-" in name_clean else name_clean
            
            # 2. Limpieza del sufijo anatómico de origen (El fix para AOMIC y similares)
            if extracted_id.endswith("_T1w"):
                extracted_id = extracted_id[:-4]
                
            file_map[extracted_id] = in_dir / fname

    solid_mask, _ = load_solid_mask(MNI_MASK_PATH)

    print(f"Procesando {len(df)} sujetos. Lanzando {args.n_jobs} workers en paralelo...")

    results = Parallel(n_jobs=args.n_jobs, backend="loky")(
        delayed(process_subject)(row, idx, in_dir, output_root, args, solid_mask, file_map)
