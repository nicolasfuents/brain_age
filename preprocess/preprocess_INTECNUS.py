...
def process_subject(row, idx, output_root, dir_filter):
    ...
    # Lógica relajada para INTECNUS: Buscar cualquier NIfTI en la carpeta del sujeto
    if dir_filter:
        ...
        nifti_files = list(raw_path_dir.rglob("*.nii")) + list(raw_path_dir.rglob("*.nii.gz"))
    
    if len(nifti_files) == 0: 
        return f"[ERROR] {subject_id}: Imagen NIfTI no encontrada"
    elif len(nifti_files) > 1:
        raw_path = nifti_files[0]
        warning_msg = f" (ADVERTENCIA: Múltiples NIfTIs encontrados. Usando {raw_path.name})"
    else:
        raw_path = nifti_files[0]
        warning_msg = ""
