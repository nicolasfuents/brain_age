...
# Ejemplo de uso:
# ~/miniforge3/envs/gpu_env/bin/python explain_clinical_v3.py --axial-id 20250822-1437 --coronal-id 20250814-0853 --sagittal-id 20250822-1437 --planes axial coronal sagittal --n 10 --slice-mode middle --ig-steps 96 --ig-baseline mean --occ-patch 12 --occ-stride 8 --occ-baseline zero --gradcam global --lrp on --alpha 0.7 --topk 10 --outdir ./explanations_clinical_promedio_LRP_10
# ---------------------------------------------------------
...
# -------------------- LRP (Zennit) --------------------
def _lrp_composite():
    return EpsilonPlus(epsilon=1e-6)

def lrp_signed(model, image, which="global"):
    """
    LRP con Zennit (EpsilonPlus). Si falla (p.ej. por diferencias de versión),
    hace fallback a grad*input firmado.
    """
    model_for_lrp = _OnlyGlobal(model).eval()
    comp = _lrp_composite()

    # Asegura tensor rastreable
    x = image.clone().detach().requires_grad_(True)

    # Target explícito: suma de la salida -> escalar
    def _target_sum(o):
        return o.sum()

    try:
        with ZGradient(model_for_lrp, composite=comp) as attr:
            y, R = attr(x, target=_target_sum)   # <-- clave: usar 'target', NO 'grad_output'

        # Zennit puede devolver lista/tupla; normaliza a tensor
        if isinstance(R, (list, tuple)):
            R = R[0]

        # Esperamos R con forma 1xCxHxW (como grad w.r.t. input)
        rel = R.detach().cpu().sum(dim=1)[0].numpy()  # -> HxW
        m = np.nanmax(np.abs(rel))
        return (rel / m) if (m is not None and m > 0) else rel

    except Exception:
        # Fallback robusto: grad*input firmado y normalizado a [-1,1]
        out = _score_scalar(model, x, which="global")
        model.zero_grad(set_to_none=True)
        out.backward()
        sal = (x.grad * x).detach().cpu().sum(dim=1)[0].numpy()  # HxW
        m = np.nanmax(np.abs(sal))
        return (sal / m) if (m is not None and m > 0) else sal
...
def run_for_plane(plane, args, subject_id, ensemble_title=None):
    ...
    if args.ig_record_steps:
        ...
        for k, f in enumerate(frames):
            ...
            _overlay_and_save(f["base_np"], f["ig_map"], p_ov, alpha=args.alpha,
                              title=f"IG ({plane})", show_colorbar=False, cmap="seismic", vmin=-1, vmax=1,
    ...
    # LRP
    metrics_lrp = {}
    if getattr(args, "lrp", "on") == "on":
        try:
            lrp = lrp_signed(model, image, which="global")
            np.save(os.path.join(outdir, "lrp.npy"), lrp)
            for base_i, lab in zip(bases, labels):
                _overlay_and_save(base_i, lrp, os.path.join(outdir, f"lrp_{lab}.png"),
                                  alpha=args.alpha, title=_fmt_title("LRP", plane, age_txt, pred_txt),
                                  cmap="seismic", vmin=-1, vmax=1, mask_2d=mask_mid, apply_mask=use_mask)
                _overlay_and_save(base_i, lrp, os.path.join(outdir, f"lrp_{lab}_nobar.png"),
                                  title=_fmt_title("LRP", plane, age_txt, pred_txt, for_ensemble=True),
                                  show_colorbar=False, cmap="seismic", vmin=-1, vmax=1,
                                  mask_2d=mask_mid, apply_mask=use_mask)
            metrics_lrp = _roi_metrics_from_heatmap(lrp, atlas_mid[None, ...])
            ordered_lrp = _write_roi_csv(os.path.join(outdir, "roi_metrics_lrp.csv"), metrics_lrp, id2label, topk=None)
            _draw_topk_contours(base_mid, lrp, atlas_mid, ordered_lrp,
                os.path.join(outdir, f"contours_lrp_top{args.topk}.png"),
                k=args.topk, alpha=args.alpha, signed=True,
                mask_2d=mask_mid, apply_mask=use_mask, plane=plane)

        except Exception as e:
            print(f"[WARN] LRP falló en {plane}: {e}")

    plane_metrics = {"ig": metrics_ig, "occlusion": metrics_occ}
    if getattr(args, "lrp", "on") == "on":
        plane_metrics["lrp"] = metrics_lrp


    # --------- Grad-CAM firmado (GLOBAL) ----------
    if args.gradcam == "global":
        gc = gradcam_signed(model, image, which="global")
        np.save(os.path.join(outdir, "gradcam_global.npy"), gc)
        for base_i, lab in zip(bases, labels):
            _overlay_and_save(base_i, gc,
                os.path.join(outdir, f"gradcam_global_{lab}.png"),
                alpha=args.alpha, title=_fmt_title("Grad-CAM", plane, age_txt, pred_txt),
                cmap="seismic", vmin=-1, vmax=1, mask_2d=mask_mid, apply_mask=use_mask)
            _overlay_and_save(base_i, gc,
                os.path.join(outdir, f"gradcam_global_{lab}_nobar.png"),
                title=_fmt_title("Grad-CAM", plane, age_txt, pred_txt, for_ensemble=True),
                show_colorbar=False, cmap="seismic", vmin=-1, vmax=1,
                mask_2d=mask_mid, apply_mask=use_mask)
    ...
    if getattr(args, "lrp", "on") == "on":
        paths = _collect_paths("lrp", [outdir])
        if any(p is not None for p in paths):
            _grid_save_shared_colorbar_nobar(paths, os.path.join(outdir, "lrp_montage.png"),
                                             title=ensemble_title, signed=True)
...
# -------------------- CLI --------------------
def main():
    ap = argparse.ArgumentParser(description="Explicabilidad clínica mínima: IG firmado + Occlusion firmado + Grad-CAM global + LRP")
    # ids / rutas
    ...
    # LRP
    ap.add_argument("--lrp", choices=["on","off"], default="on")
    ...
    # Acumulador poblacional de métricas por ROI
    population_acc = {
    ...
    if args.lrp == "on":
        population_acc["lrp"] = defaultdict(lambda: {"sum_abs":0.0,"sum_pos":0.0,"cnt_pos":0,"sum_neg":0.0,"cnt_neg":0,"area_px":0})

    methods_all = ["ig", "occlusion"] + (["lrp"] if args.lrp=="on" else []) + (["gradcam_global"] if args.gradcam=="global" else [])
    avg_sum   = {pl:{m:None for m in methods_all} for pl in args.planes}
    avg_count = {pl:{m:None for m in methods_all} for pl in args.planes}
    avg_base  = {pl: None for pl in args.planes}   # base media para overlay (slice medio)
    base_n    = {pl: 0 for pl in args.planes}
    ...
    for subject_id in ids_to_use:
        ...
        # Inicializar acumulador GLOBAL por SUJETO (no por plano)
        global_acc = {
            "ig": defaultdict(lambda: {"sum_abs":0.0,"sum_pos":0.0,"cnt_pos":0,"sum_neg":0.0,"cnt_neg":0,"area_px":0}),
            "occlusion": defaultdict(lambda: {"sum_abs":0.0,"sum_pos":0.0,"cnt_pos":0,"sum_neg":0.0,"cnt_neg":0,"area_px":0})
        }
        if args.lrp == "on":
            global_acc["lrp"] = defaultdict(lambda: {"sum_abs":0.0,"sum_pos":0.0,"cnt_pos":0,"sum_neg":0.0,"cnt_neg":0,"area_px":0})

        for plane in args.planes:
            outdir, plane_metrics = run_for_plane(plane, args, subject_id, ensemble_title=title)
            # --- Acumular mapas por plano ---
            sample_p = torch.load(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                               f"../../data/processed/{plane}", f"{subject_id}.pt"),
                                  map_location="cpu")
            imgC = sample_p["image"]; mid = imgC.shape[0]//2
            base_mid = imgC[mid].numpy().astype(np.float32)
            b = base_mid - base_mid.min()
            if b.max() > 0: b = b / b.max()
            avg_base[plane] = b if avg_base[plane] is None else (avg_base[plane] + b)
            base_n[plane] += 1

            for m, fname in [("ig","ig.npy"), ("occlusion","occlusion.npy")] + ([("lrp","lrp.npy")] if args.lrp=="on" else []):
                fpath = os.path.join(outdir, fname)
                if not os.path.exists(fpath):
                    print(f"[WARN] {m} ausente en {plane}: {fpath}")
                    continue
                h = np.load(fpath).astype(np.float32)
                valid = ~np.isnan(h)
                if avg_sum[plane][m] is None:
                    avg_sum[plane][m]   = np.where(valid, h, 0.0)
                    avg_count[plane][m] = valid.astype(np.int32)
                else:
                    avg_sum[plane][m]   += np.where(valid, h, 0.0)
                    avg_count[plane][m] += valid.astype(np.int32)
            ...
            if args.gradcam == "global":
                m = "gradcam_global"
                f = os.path.join(outdir, "gradcam_global.npy")
                if os.path.exists(f):
                    ...
                    print(f"[WARN] gradcam_global ausente en {plane}: {f}")

            plane_outdirs.append(outdir)
            
            for method in (["ig","occlusion"] + (["lrp"] if args.lrp=="on" else [])):
                for rid, d in plane_metrics[method].items():
                    ...
                    # acumular también para promedio poblacional
                    gpop = population_acc[method][rid]
                    gpop["sum_abs"] += d["sum_abs"]
                    gpop["area_px"] += d["area_px"]
                    if "sum_pos" in d: gpop["sum_pos"] += d["sum_pos"]
                    if "cnt_pos" in d: gpop["cnt_pos"] += d["cnt_pos"]
                    if "sum_neg" in d: gpop["sum_neg"] += d["sum_neg"]
                    if "cnt_neg" in d: gpop["cnt_neg"] += d["cnt_neg"]
        ...
        for kind, signed in [("ig", True), ("occlusion", True)] + ([("lrp", True)] if args.lrp=="on" else []):
        ...
        if plane_outdirs:
            ...
            def _finalize_global(method_name):
                ...
                for plane, p_outdir in zip(args.planes, plane_outdirs):
                    ...
                    fheat = os.path.join(p_outdir, f"{method_name}.npy")
                    if not os.path.exists(fheat):
                        print(f"[WARN] {method_name} ausente en {plane}: {fheat}")
                        continue
                    heat = np.load(fheat)
            ...
            if args.lrp=="on":
                _finalize_global("lrp")


    # --- Cerrar promedios poblacionales ---
    avg_dir = os.path.join(args.outdir, "averages"); os.makedirs(avg_dir, exist_ok=True)
    methods_all = ["ig", "occlusion"] + (["lrp"] if args.lrp=="on" else []) + (["gradcam_global"] if args.gradcam=="global" else [])


    def _nanmean_from_sum_count(S, C):
        mm = np.where(C > 0, S / np.maximum(C, 1), np.nan).astype(np.float32)
        return np.clip(mm, -1.0, 1.0)

    per_method_paths = {m: [] for m in methods_all}

    for plane in args.planes:
        # base promedio para overlay por plano
        if avg_base[plane] is None:
            for m in methods_all: per_method_paths[m].append(None)
            continue
        base_mean = (avg_base[plane] / max(base_n[plane], 1)).astype(np.float32)

        for m in methods_all:
            if avg_sum[plane][m] is None:
                per_method_paths[m].append(None)
                continue

            avg_map = _nanmean_from_sum_count(avg_sum[plane][m], avg_count[plane][m])

            # guardar NPY y PNG sin barra (para montaje con barra compartida)
            np.save(os.path.join(avg_dir, f"{m}_avg_{plane}.npy"), avg_map)
            out_png_nobar = os.path.join(avg_dir, f"{m}_avg_{plane}_nobar.png")
            _overlay_and_save(
                base_mean, avg_map, out_png_nobar,
                alpha=args.alpha, title=f"{m.upper()} promedio ({plane})",
                show_colorbar=False, cmap="seismic", vmin=-1, vmax=1,
                mask_2d=(base_mean > 0).astype(np.uint8), apply_mask=(not args.no_mask)
            )
            per_method_paths[m].append(out_png_nobar)

    titles = {
        "ig": "Promedio poblacional — Integrated Gradients",
        "occlusion": "Promedio poblacional — Occlusion",
        "lrp": "Promedio poblacional — LRP",
        "gradcam_global": "Promedio poblacional — Grad-CAM (global)"
    }
    for m in methods_all:
        paths = per_method_paths[m]
        if any(p is not None for p in paths):
            _grid_save_shared_colorbar_nobar(
                paths, os.path.join(avg_dir, f"{m}_avg_ensemble.png"),
                title=titles[m], signed=True
            )
    # === [ADD] CSVs de 'averages' por método (mismas métricas que ensemble) ===
    labels_csv = _find_labels_csv(base_dir)
    id2label = _load_labels_map(labels_csv)

    def _finalize_population(method_name):
        rows = []
        for rid, d in population_acc[method_name].items():
            mean_pos = (d["sum_pos"]/d["cnt_pos"]) if d["cnt_pos"]>0 else 0.0
            mean_neg = (d["sum_neg"]/d["cnt_neg"]) if d["cnt_neg"]>0 else 0.0
            mean_net = mean_pos + mean_neg
            density  = (d["sum_abs"]/d["area_px"]) if d["area_px"]>0 else 0.0
            pos_abs  = d["sum_pos"]; neg_abs = -d["sum_neg"]; denom = pos_abs + neg_abs
            ratio_pos = (pos_abs/denom) if denom>0 else 0.0
            rows.append((rid, id2label.get(rid, f"ROI_{rid}"),
                         mean_pos, mean_neg, mean_net,
                         d["sum_abs"], d["area_px"], density, ratio_pos))
        rows.sort(key=lambda t: t[7], reverse=True)

        out_dict = {
            r[0]: {
                "mean_pos": r[2], "mean_neg": r[3], "mean_net": r[4],
                "sum_abs": r[5], "area_px": r[6],
                "density_abs": r[7], "ratio_pos": r[8]
            } for r in rows
        }
        _write_roi_csv(os.path.join(avg_dir, f"roi_metrics_{method_name}_averages.csv"),
                       out_dict, id2label, topk=None)

    for method_name in (["ig","occlusion"] + (["lrp"] if args.lrp=="on" else [])):
        _finalize_population(method_name)
